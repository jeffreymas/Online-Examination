...
namespace ExamOnline.Repositories.Data
{
    public class QuestionRepo : GeneralRepo<Question, MyContext>
    {
        ...
        public override async Task<List<Question>> GetAll()
        {
            ...
            var data = await _context.Question.Include("Subjects").Include("Section").Where(x => x.isDelete == false).ToListAsync();
            if (data.Count == 0)
            {
                return null;
            }
        ...
        {
            var data = await _context.Question.Include("Subjects").Include("Section").SingleOrDefaultAsync(x => x.Id == Id && x.isDelete == false);
            if (!data.Equals(0))
            {
                return data;
            }
